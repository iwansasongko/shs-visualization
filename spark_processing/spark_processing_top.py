...
def main():
    ...
    try:
        ...
        yearly_stats = gpu_df.withColumn("year", year("date")) \
            .groupBy("year", "name") \
            .agg(
        ...
        window_spec = Window.partitionBy("year").orderBy(desc("frequency"), desc("avg_usage"))

        result = yearly_stats.withColumn("rank", row_number().over(window_spec)) \
            .filter(col("rank") <= 5) \
            .drop("rank", "frequency")
